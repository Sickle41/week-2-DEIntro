...
def fetch_object(key: str, dest_dir: Path = RAW_DIR) -> Path:
    ...
    client.download_file(settings.bucket, key, str(dest_path))
...
def fetch_all(dest_dir: Path = RAW_DIR) -> dict[str, Path]:
    ...
    return {
        "orders": fetch_object(settings.orders_key, dest_dir),
        "customers": fetch_object(settings.customers_key, dest_dir),
    }
